...
def init_db():
    ...
    # === NOVA TABELA: recursos_extra ===
    cur.execute("""CREATE TABLE IF NOT EXISTS recursos_extra (
        id INTEGER PRIMARY KEY AUTOINCREMENT,
        conta_receber_id INTEGER,
        valor REAL,
        descricao TEXT,
...
# === FUNCOES AUXILIARES: RECURSOS EXTRA ===
def get_total_extra(conta_id):
    conn = get_conn()
    row = conn.execute("SELECT COALESCE(SUM(valor), 0) FROM recursos_extra WHERE conta_receber_id=?", (conta_id,)).fetchone()
    conn.close()
    return row[0] if row else 0.0

def get_extras(conta_id):
    conn = get_conn()
    rows = conn.execute("SELECT id, valor, descricao, data_add, created_at FROM recursos_extra WHERE conta_receber_id=? ORDER BY id DESC", (conta_id,)).fetchall()
    conn.close()
    return rows

def get_valor_original(conta_id, valor_total):
    return valor_total - get_total_extra(conta_id)
...
if "show_form_extra" not in st.session_state:
    st.session_state["show_form_extra"] = None
...
def tela_login():
    st.markdown(
        ...
        .login-title { font-size: 84px; font-weight: 800; text-align: center; background: linear-gradient(135deg, #38bdf8, #818cf8, #c084fc); -webkit-background-clip: text; -webkit-text-fill-color: transparent; letter-spacing: 4px; margin: 0; }
...
def pagina_esfera_detalhe():
    ...
    titulo = f"Esfera {esfera}" if esfera in ("Federal", "Estadual", "Municipal") else esfera
    st.markdown(f'<p class="mm-title">{titulo}</p>', unsafe_allow_html=True)
    st.markdown('<p class="mm-subtitle">Detalhamento das contas cadastradas</p>', unsafe_allow_html=True)

    conn = get_conn()
    total = conn.execute("SELECT COALESCE(SUM(valor_total),0) FROM contas_receber WHERE esfera=?", (esfera,)).fetchone()[0]
    linhas = conn.execute("SELECT id, numero_conta, fonte, tipo_recurso, valor_total, data_recebimento, programa_politica, setor_gasto FROM contas_receber WHERE esfera=? ORDER BY id DESC", (esfera,)).fetchall()
    conn.close()

    st.markdown(f'<div class="mm-card" style="margin-bottom:20px;"><div class="mm-card-label">Total da Esfera</div><div class="mm-card-value">{format_currency(total)}</div></div>', unsafe_allow_html=True)

    if not linhas:
        st.info("Nenhuma conta cadastrada para esta esfera ainda.")
    else:
        for linha in linhas:
            cid, numero_conta, fonte, tipo_recurso, valor_total, data_recebimento, programa, setor = linha
            extra_total = get_total_extra(cid)
            valor_orig = get_valor_original(cid, valor_total)

            st.markdown(f'<div class="mm-card" style="text-align:left;margin-bottom:10px;"><strong>Conta {numero_conta}</strong> - Fonte {fonte} - {tipo_recurso}<br>'
                        f'<span style="color:#94a3b8;font-size:12px;">Programa: {programa or "-"} | Setor: {setor or "-"} | Data: {data_recebimento or "-"}</span><br>'
                        f'<span style="color:#94a3b8;">Valor Original:</span> <span style="color:#f1f5f9;">{format_currency(valor_orig)}</span> &nbsp;|&nbsp; '
                        f'<span style="color:#7dd3fc;">Recursos Extra:</span> <span style="color:#7dd3fc;">{format_currency(extra_total)}</span> &nbsp;|&nbsp; '
                        f'<span style="color:#38bdf8;font-weight:700;">Valor Atual:</span> <span style="color:#38bdf8;font-weight:700;">{format_currency(valor_total)}</span></div>',
                        unsafe_allow_html=True)

            # Botao Adicionar Recurso Extra
            if st.button(f"➕ Adicionar Recurso Extra - Conta {numero_conta}", key=f"btn_extra_{cid}", use_container_width=True):
                st.session_state["show_form_extra"] = cid
                st.rerun()

            # Formulario de recurso extra
            if st.session_state.get("show_form_extra") == cid:
                with st.form(key=f"form_extra_{cid}"):
                    st.markdown("**Adicionar Recurso Extra**")
                    valor_extra = st.text_input("Valor (R$)", key=f"val_extra_{cid}")
                    descricao = st.text_input("Descricao", key=f"desc_extra_{cid}")
                    ...
                    if salvar:
                        ...
                        if v <= 0:
                            ...
                            conn.execute("INSERT INTO recursos_extra (conta_receber_id, valor, descricao, data_add, created_at) VALUES (?, ?, ?, ?, ?)",
                                         (cid, v, descricao, str(data_add), now))
                            conn.execute("UPDATE contas_receber SET valor_total = valor_total + ? WHERE id = ?", (v, cid))
                            conn.commit()
                            conn.close()
                            st.session_state["show_form_extra"] = None
            ...
            # Historico de recursos extras da conta
            extras = get_extras(cid)
            if extras:
                with st.expander(f"📋 Historico de Recursos Extras - Conta {numero_conta}"):
                    for ex in extras:
                        st.markdown(f"**{format_currency(ex[1])}** | {ex[2] or 'Sem descricao'} | Data: {ex[3]} | Registrado: {ex[4]}")
...
def pagina_cadastro_contas():
    ...
    st.markdown(f'<div class="mm-card" style="text-align:left;margin-bottom:16px;padding:14px 18px;"><div class="mm-card-label">Fonte Vinculada</div><div class="mm-card-value" style="font-size:20px;">{fonte_vinculada}</div></div>', unsafe_allow_html=True)
...
def pagina_contas_cadastradas():
    ...
    st.markdown('<p class="mm-subtitle">Consulte todas as contas registradas</p>', unsafe_allow_html=True)
    conn = get_conn()
    linhas = conn.execute("SELECT id, esfera, numero_conta, fonte, tipo_recurso, valor_total, data_recebimento FROM contas_receber ORDER BY id DESC").fetchall()
    conn.close()
    if not linhas:
    ...
    for linha in linhas:
        ...
        st.markdown(f'<div class="mm-card" style="text-align:left;margin-bottom:10px;"><strong>{esfera}</strong> - Conta {numero_conta} - Fonte {fonte}<br>{tipo_recurso} - {format_currency(valor_total)} - {data_recebimento}</div>', unsafe_allow_html=True)

def pagina_realizar_compras():
    st.markdown('<p class="mm-title">Realizar Compras</p>', unsafe_allow_html=True)
    st.markdown('<p class="mm-subtitle">Registre ordens de compra</p>', unsafe_allow_html=True)
    conn = get_conn()
    contas = conn.execute("SELECT id, esfera, numero_conta, fonte, valor_total FROM contas_receber ORDER BY id DESC").fetchall()
    conn.close()
    if not contas:
        st.info("Cadastre uma conta antes.")
        return
    opcoes = {f"{c[1]} - Conta {c[2]}": c for c in contas}
    escolha = st.selectbox("Selecione a conta", list(opcoes.keys()))
    conta_selecionada = opcoes[escolha]
    with st.form("form_compra"):
        ficha = st.text_input("Ficha")
        tipo_despesa = st.selectbox("Tipo", ["Material de Consumo", "Servico PF", "Servico PJ", "Distribuicao Gratuita", "Equipamento"])
        data_compra = st.date_input("Data", value=datetime.now())
        valor_compra = st.text_input("Valor", value="R$ 0,00")
        ...
        if registrar:
            ...
            if vc <= 0:
                ...
                conn = get_conn()
                conn.execute("INSERT INTO ordens_compra (conta_receber_id, esfera, numero_conta, fonte, ficha, tipo_despesa, data_compra, valor_compra, produto_servico, created_at) VALUES (?,?,?,?,?,?,?,?,?,?)",
                            (conta_selecionada[0], conta_selecionada[1], conta_selecionada[2], conta_selecionada[3], ficha, tipo_despesa, data_compra.strftime("%d/%m/%Y"), vc, produto, datetime.now().strftime("%d/%m/%Y %H:%M:%S")))
                conn.commit()
                conn.close()
...
def pagina_superavit():
    ...
    st.markdown('<p class="mm-subtitle">Controle de superavit</p>', unsafe_allow_html=True)
    with st.form("form_superavit"):
        ...
        if registrar:
            ...
            if v <= 0:
                ...
                conn = get_conn()
                conn.execute("INSERT INTO superavit (esfera, fonte_original, fonte_superavit, saldo_total, saldo_restante, created_at) VALUES (?,?,?,?,?,?)",
                            (esfera, get_fonte(esfera), get_fonte_superavit(esfera), v, v, datetime.now().strftime("%d/%m/%Y %H:%M:%S")))
                conn.commit()
                conn.close()
                st.success("Superavit registrado!")
                st.rerun()
    conn = get_conn()
    ...
    if linhas:
        for linha in linhas:
            esfera, fonte_sup, saldo_total, saldo_restante, criado = linha
            st.markdown(f'<div class="mm-card" style="text-align:left;margin-bottom:10px;"><strong>{esfera}</strong> - Fonte {fonte_sup}<br>Total: {format_currency(saldo_total)} - Restante: {format_currency(saldo_restante)} - {criado}</div>', unsafe_allow_html=True)

def pagina_upload_arquivos():
    st.markdown('<p class="mm-title">Upload de Arquivos</p>', unsafe_allow_html=True)
    st.markdown('<p class="mm-subtitle">Envie documentos e comprovantes</p>', unsafe_allow_html=True)
    bloco = st.selectbox("Categoria", ["Contas", "Compras", "Outros"])
    ...
    if st.button("Enviar", use_container_width=True):
        if not arquivo:
            ...
            dados = arquivo.read()
            texto = extrair_texto(dados, arquivo.name)
            conn = get_conn()
            conn.execute("INSERT INTO arquivos_saude (bloco, nome_arquivo, conteudo_texto, dados_arquivo, data_upload) VALUES (?,?,?,?,?)",
                        (bloco, arquivo.name, texto, dados, datetime.now().strftime("%d/%m/%Y %H:%M:%S")))
            conn.commit()
            conn.close()
    ...
    if linhas:
        for linha in linhas:
            st.markdown(f'<div class="mm-card" style="text-align:left;margin-bottom:10px;"><strong>{linha[1]}</strong> - {linha[0]}<br><span style="color:#94a3b8;font-size:11px;">{linha[2]}</span></div>', unsafe_allow_html=True)

def pagina_trocar_senha():
    st.markdown('<p class="mm-title">Trocar Senha</p>', unsafe_allow_html=True)
    st.markdown('<p class="mm-subtitle">Atualize sua senha</p>', unsafe_allow_html=True)
    with st.form("form_trocar_senha"):
        senha_atual = st.text_input("Senha Atual", type="password")
        nova_senha = st.text_input("Nova Senha", type="password")
        confirmar = st.text_input("Confirmar", type="password")
        salvar = st.form_submit_button("Alterar", use_container_width=True)
        if salvar:
            conn = get_conn()
            row = conn.execute("SELECT id FROM users WHERE id=? AND password_hash=?", (st.session_state.get("usuario_id"), hash_senha(senha_atual))).fetchone()
            if not row:
