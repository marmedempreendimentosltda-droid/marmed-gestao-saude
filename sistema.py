...
def dashboard():
    ...
    conn = sqlite3.connect("marmed.db")
    c = conn.cursor()
    esferas_tela = ["REPASSE FEDERAL", "REPASSE ESTADUAL", "RECURSO MUNICIPAL", "TRANSFERÊNCIA", "TRANSPOSIÇÃO"]
    esf_keys = ["Federal", "Estadual", "Municipal", "Transferência", "Transposição"]
    cores = ["#3b82f6", "#22c55e", "#eab308", "#a855f7", "#ef4444"]
    cols = st.columns(5)
    for i, (tit, esf, cor) in enumerate(zip(esferas_tela, esf_keys, cores)):
...
def esfera_detalhe():
    ...
    if not contas: st.info(f"Nenhuma conta cadastrada para {esf}.")
...
def cadastrar_contas():
    ...
    with st.expander("NOVO CADASTRO", expanded=True):
        ...
        if esfera: st.markdown(f'<p style="color:#22d3ee;font-weight:600;">Fonte: {fonte_auto}</p>', unsafe_allow_html=True)
        ...
        if val_total_calc > 0: st.markdown(f'<p style="color:#00d4ff;font-size:16px;font-weight:700;">Valor Total: {format_currency(val_total_calc)}</p>', unsafe_allow_html=True)
...
def contas_cadastradas():
    ...
    for tab, esf in [(tab1, "Federal"), (tab2, "Estadual"), (tab3, "Municipal")]:
        with tab:
            ...
            if registros:
                ...
                for c in ["V. Custeio", "V. Invest.", "V. Total"]: pdf[c] = pdf[c].apply(lambda x: format_currency(x))
                ...
                if escolha and opcoes[escolha]:
                    ...
                    with c2:
                        if st.button("Excluir", key=f"del_{esf}_{rid}"):
                            conn.execute("DELETE FROM contas_receber WHERE id=?", (rid,)); conn.commit(); st.success("Conta excluída!"); st.rerun()
            else: st.markdown(f'<p style="color:#94a3b8;">Nenhuma conta {esf} cadastrada.</p>', unsafe_allow_html=True)
    ...
    else: st.markdown('<p style="color:#94a3b8;text-align:center;">Nenhum superávit registrado.</p>', unsafe_allow_html=True)
    if st.button("MIGRAR SALDOS PARA SUPERÁVIT", key="migrar_superavit"):
        for esf in ["Federal", "Estadual"]:
            ...
            if total > 0:
                fonte_orig = get_fonte(esf); fonte_sup = get_fonte_superavit(esf)
                ...
                else: conn.execute("INSERT INTO superavit (esfera, fonte_original, fonte_superavit, saldo_total, saldo_restante, created_at) VALUES (?,?,?,?,?,?)", (esf, fonte_orig, fonte_sup, total, total, agora))
...
def editar_conta():
    ...
    with c1:
        if st.button("Salvar Alterações"):
            ...
            conn.commit(); conn.close(); st.success("Conta atualizada!"); st.session_state["page"] = "CONTAS CADASTRADAS"; st.rerun()
    with c2:
        if st.button("Voltar"): st.session_state["page"] = "CONTAS CADASTRADAS"; st.rerun()
    conn.close()
...
def realizar_compras():
    ...
    if ordens:
        ...
        if escolha_ordem and opcoes_ordem[escolha_ordem]:
            ...
            c1, c2 = st.columns(2)
            with c1:
                if st.button("Editar", key=f"edit_ordem_{oid}"): st.session_state["edit_ordem_id"] = oid; st.session_state["page"] = "EDITAR ORDEM COMPRA"; st.rerun()
            with c2:
                if st.button("Excluir", key=f"del_ordem_{oid}"): conn.execute("DELETE FROM ordens_compra WHERE id=?", (oid,)); conn.commit(); st.success("Solicitação excluída!"); st.rerun()
    else: st.markdown('<p style="color:#94a3b8;">Nenhuma solicitação registrada.</p>', unsafe_allow_html=True)
    ...
    for tab, esf in [(tab1, "Federal"), (tab2, "Estadual"), (tab3, "Municipal")]:
        with tab:
            ...
            if not contas: st.markdown(f'<p style="color:#94a3b8;">Nenhuma conta {esf} disponível.</p>', unsafe_allow_html=True)
            else:
                for cid, esfera, num, fonte, trecurso, vtotal in contas:
                    ...
                    with st.expander(f"{num} - Fonte {fonte}"):
                        ...
                        if esf != "Municipal":
                            ...
                            if saldo_sup > 0: st.warning(f"Superávit de {format_currency(saldo_sup)} para {esf}. Utilize-o antes.")
                        with st.form(key=f"form_compra_{esf}_{cid}"):
                            ...
                            with col2:
                                ...
                                if valor_compra > saldo: st.markdown(f'<p style="color:#ef4444;font-size:12px;">Excede saldo de {format_currency(saldo)}</p>', unsafe_allow_html=True)
                            ...
                            if st.form_submit_button(f"Solicitar Compra - {num}"):
                                ...
                                if erros: st.error(f"Preencha: {', '.join(erros)}")
                                    ...
                                    conn.commit(); st.success(f"Solicitação registrada!"); st.rerun()
    conn.close()
...
def editar_ordem_compra():
    ...
    if not oid: st.error("Nenhuma selecionada.")
    conn = sqlite3.connect("marmed.db")
    row = conn.execute("SELECT * FROM ordens_compra WHERE id=?", (oid,)).fetchone()
    if not row: conn.close(); st.error("Não encontrada."); return
...
def change_password():
    ...
    if st.button("Salvar nova senha"):
        if new_pass != confirm: st.error("As senhas não conferem")
        else:
            ...
            if row:
                ...
                conn.commit(); conn.close(); st.success("Senha alterada com sucesso")
...
def bloco_saude(titulo, sigla, explicacao, site_url, cor_topo="#1e40af", esfera_rel=None):
    ...
    with st.expander(f"📋 {sigla} - {titulo}", expanded=False):
        ...
        if uploaded:
            ...
            conn.commit(); st.success(f"Arquivo '{uploaded.name}' anexado!"); st.rerun()
        ...
        if arquivos:
            st.markdown(f'<p style="color:#94a3b8;font-size:13px;">{len(arquivos)} arquivo(s) anexado(s)</p>', unsafe_allow_html=True)
            for arq in arquivos:
            ...
            st.markdown(f'<p style="color:#7dd3fc;font-size:14px;margin-top:12px;font-weight:600;">🔍 Pesquisar nos documentos</p>', unsafe_allow_html=True)
                ...
                for arq in arquivos:
                    ...
                    if matches:
                        ...
                        for m in matches[:5]:
                            ...
                            trecho_destacado = re.sub(re.escape(termo), f'<span style="background:#fbbf24;color:#000;padding:2px 4px;border-radius:3px;font-weight:700;">{termo}</span>', trecho, flags=re.IGNORECASE)
                            st.markdown(f'<div style="background:rgba(15,23,42,0.5);border-radius:6px;padding:10px;margin-bottom:6px;color:#cbd5e1;font-size:13px;font-family:monospace;line-height:1.5;">...{trecho_destacado}...</div>', unsafe_allow_html=True)
                if not encontrou: st.markdown(f'<p style="color:#94a3b8;">Nenhuma ocorrência de "{termo}".</p>', unsafe_allow_html=True)
        else: st.markdown('<p style="color:#64748b;font-size:13px;">Nenhum documento anexado. Faça o upload acima.</p>', unsafe_allow_html=True)
        conn.close()
